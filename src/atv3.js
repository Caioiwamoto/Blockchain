var sha256 = function sha256(ascii) { //código referenciado para o cálculo do hash
...
//O código tentará seguir uma ordem cronológica das etapas
function getNum(message){ //aqui é determinada a dificuldade
...
function calculoHash(indexbloco,dado,ultimoHash,nonce) { //calcula o hash usando a função sha256
    return sha256(indexbloco + dado + ultimoHash + nonce).toString();
}

function mineraBloco(indexbloco, dado, ultimoHash){ //minera o bloco a partir da dificuldade escolhida
	var flag = 1;
	var nonce = -1;
	do {
      nonce++;
...
function validaChain(){ //essa função faz alterações, validações e também chama a mineração
	var tamChain = $('[id^="blocos"]').length;
	for(i=0;i<tamChain;i++){ //percorre todos os blocos existentes
		var vetorHash = $('[id^="hash'+i+'"]').text(); //recebe o o valor do hash
		var vetorDado = $('[id^="dado'+i+'"]').val(); //recebe o valor do dado
		if(vetorDado==""){ //verifica se há algo no campo dado caso ele tenha sido alterado
